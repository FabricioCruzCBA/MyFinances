                            ...
                            @if(count($orcamento)>0)        
                                @foreach($orcamento as $dados)
                                    <tr>
                                        ...
                                        <td>
                                            <input type="hidden" id="id{{$dados->id}}" name="id[{{$dados->id}}]" value="{{$dados->id}}">
                                            <div class="input-group mb-3">
                                                <span class="input-group-text">R$</span> 
                                                <input type="text" class="form-control Numero" aria-label="Valor" id="valor{{$dados->id}}" name="valor[{{$dados->id}}]" value="{{$dados->ValorItemOrc}}"> 
                                ...
                                @foreach($categoria as $itens)
                                    @if(count($itens->categoriaCategoriaorcamento)==0)
                                        <tr>
                                            <td>{{$itens->NomeCategoria}}</td>
                                            <td>
                                                <div class="row">
                                                    <div class="col-2 col-lg-2">
                                                        <button class="btn @if($itens->TipoCategoria == 'D') btn-despesa @else btn-receita @endif " title="Visualizar"><i class="bi bi-{{$itens->IconeCategoria}}"></i> </button>
                                                    </div>
                                                </div>
                                            </td>
                                            <td>
                                                <input type="hidden" id="idNew{{$itens->id}}" name="idNew[{{$itens->id}}]" value="{{$itens->id}}">
                                                <div class="input-group mb-3">
                                                    <span class="input-group-text">R$</span> 
                                                    <input type="text" class="form-control Numero" aria-label="Valor" id="valorNew{{$itens->id}}" name="valorNew[{{$itens->id}}]" value="0"> 
                                                    <span class="input-group-text">,00</span>
                                                </div>
                                            </td>
                                        </tr>   
